...
# ~/.dotfiles is a disposable ARTIFACT of this repo — deliberately NOT a git repo:
# the sources of truth are this checkout (public) and, optionally, the private
# overlay at ~/.dotfiles.local (see README "Extending"), which staging never reads.
DOTREPO="$HOME/.dotfiles"
OVERLAY="$HOME/.dotfiles.local"   # optional private layer; only ever consumed via hooks below
...
for f in starship.toml ghostty.config zellij.kdl zshrc-additions.zsh gitconfig-devbook AGENTS.md; do
  if [[ -f "$DOTDIR/$f" ]]; then
    # Staged copies are kept read-only so editing an artifact fails loudly instead
    # of being silently clobbered next run. ORDER-COUPLED under `set -e`: the lift
    # (chmod u+w) must precede cp and only fire when the target exists (first run
    # has nothing to lift) — reordering this block breaks every re-run.
    [[ -e "$DOTREPO/$f" ]] && chmod u+w "$DOTREPO/$f"
    cp "$DOTDIR/$f" "$DOTREPO/$f"
    chmod a-w "$DOTREPO/$f"
  fi
done

DIVERGED_LIST=""   # takeover records, written to the manifest so divergence outlives stdout
link() { # $1 = source (in $DOTREPO), $2 = destination
  if [[ -f "$1" ]]; then
    if [[ -e "$2" && ! -L "$2" ]]; then
      # Takeover contract: a REAL file at the destination means the user owns this
      # config now — skip it, never clobber. Delete the file and re-run to return
      # to the devbook baseline. (Whole-file configs have no include mechanism, so
      # divergence-by-replacement is the supported way to customize them.)
      warn "diverged: $2 is user-owned — baseline not applied"
      DIVERGED_LIST="${DIVERGED_LIST}  $2"$'\n'
      return 0
...
if command -v git >/dev/null 2>&1; then
  ...
  # Private overlay git config: a SECOND include, gated on the overlay file existing,
  # so machine-personal git settings never live in this public repo. Guarded add
  # (grep first) keeps re-runs from duplicating the line; the elif is the teardown —
  # overlay gone ⇒ the stale include is unwired, so a re-run restores the exactly-
  # public state instead of leaving a dead reference behind.
  if [[ -f "$OVERLAY/gitconfig" ]]; then
    if git config --global --get-all include.path 2>/dev/null | grep -qF "$OVERLAY/gitconfig"; then
      ok "overlay git include already wired"
    else
      info "Adding include.path $OVERLAY/gitconfig to git global config"
      git config --global --add include.path "$OVERLAY/gitconfig"
    fi
  elif git config --global --get-all include.path 2>/dev/null | grep -qF "$OVERLAY/gitconfig"; then
    info "Removing stale overlay git include (overlay absent)"
    git config --global --fixed-value --unset include.path "$OVERLAY/gitconfig" \
      || warn "could not unwire overlay include — remove it from ~/.gitconfig by hand"
  fi
fi
...
if [[ -f "$DOTREPO/AGENTS.md" ]]; then
  # Private overlay composition: the EFFECTIVE brief = public defaults + optional
  # private additions from the overlay. The composite lives OUTSIDE ~/.dotfiles on
  # purpose — the staged tree stays a purely public artifact you can diff against
  # this repo, while the composite may hold private content (hence chmod 600).
  # Overlay absent ⇒ any stale composite is REMOVED (it can carry private content;
  # leaving it would survive an overlay teardown) and the public brief stands alone.
  COMPOSED="$HOME/.config/dev-env/AGENTS.composed.md"
  mkdir -p "$(dirname "$COMPOSED")"
  BRIEF="$DOTREPO/AGENTS.md"
  if [[ -f "$OVERLAY/AGENTS.local.md" ]]; then
    {
      printf '<!-- GENERATED by devbook setup.sh — do not edit; changes are lost on the next run. -->\n'
      printf '<!-- sources: %s (public) + %s (private) -->\n\n' "$DOTREPO/AGENTS.md" "$OVERLAY/AGENTS.local.md"
      cat "$DOTREPO/AGENTS.md"
      printf '\n'
      cat "$OVERLAY/AGENTS.local.md"
    } > "$COMPOSED"
    chmod 600 "$COMPOSED"
    BRIEF="$COMPOSED"
    ok "composed machine brief (public + private overlay)"
  elif [[ -f "$COMPOSED" ]]; then
    rm -f "$COMPOSED"
    ok "removed stale composed brief (overlay absent)"
  fi
  ensure_marked_block "$HOME/.claude/CLAUDE.md" \
    "<!-- >>> dev-env agents >>> -->" \
    "<!-- <<< dev-env agents <<< -->" \
    "@$BRIEF"
  ok "Claude Code user memory imports $BRIEF"
fi

# Leak gate (guard in the substrate, not the prose): wire this checkout's pre-commit
# hook so a commit whose staged changes contain a private sentinel string is blocked
# before it can reach the public repo. The HOOK is public; the PATTERNS are private
# ($OVERLAY/sentinels.txt) — a public pattern list would itself be the leak. Without
# patterns the hook no-ops, so clones with no overlay are unaffected.
# Undo: git -C <this checkout> config --unset core.hooksPath
if [[ -d "$DOTDIR/.git" && -f "$DOTDIR/githooks/pre-commit" ]]; then
  if [[ "$(git -C "$DOTDIR" config --local core.hooksPath 2>/dev/null || true)" == "githooks" ]]; then
    ok "sentinel pre-commit hook already wired"
  else
    info "Wiring sentinel pre-commit hook (core.hooksPath=githooks)"
    git -C "$DOTDIR" config --local core.hooksPath githooks
  fi
fi

# Private overlay hand-off: one command still sets up the WHOLE machine when a
# private layer exists. Existence-gated (strangers unaffected) and contained — a
# broken overlay script warns but can never abort devbook's own, reviewed setup.
# Skip with SKIP_LOCAL=1 to exercise devbook in isolation while debugging.
if [[ "${SKIP_LOCAL:-0}" == "1" ]]; then
  ok "private overlay hand-off skipped (SKIP_LOCAL=1)"
elif [[ -x "$OVERLAY/setup.sh" ]]; then
  info "Running private overlay setup: $OVERLAY/setup.sh"
  "$OVERLAY/setup.sh" || warn "overlay setup exited nonzero (devbook setup itself completed)"
fi
...
mkdir -p "$(dirname "$MANIFEST")"
{
  ...
  # Overlay state + divergences land here so a run is reconstructible from its
  # artifacts alone — stdout warnings scroll away; the manifest doesn't.
  if [[ -f "$OVERLAY/AGENTS.local.md" ]]; then
    echo "overlay:         present ($OVERLAY; brief composed)"
  elif [[ -d "$OVERLAY" ]]; then
    echo "overlay:         present ($OVERLAY; no AGENTS.local.md)"
  else
    echo "overlay:         absent"
  fi
  if [[ -n "$DIVERGED_LIST" ]]; then
    printf 'diverged (user-owned, baseline not applied):\n%s' "$DIVERGED_LIST"
  fi
